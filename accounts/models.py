...
class Account(models.Model):
    ...
    qualified_f = models.BooleanField(
        default=False, help_text="Tax status - set flag if account is qualified."
    )
...
class CashRecord(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.account} {self.d} {self.category} {self.description} "
            f"{self.amt} {self.cleared_f} {self.ignored}"
        )


@lru_cache(maxsize=10)
def get_cash_df(a=None, d=None, pivot=False, active_f=True, cleared=False):
...
def copy_cash_df(d=None, a=None, pivot=False, active_f=True, cleared=False):
